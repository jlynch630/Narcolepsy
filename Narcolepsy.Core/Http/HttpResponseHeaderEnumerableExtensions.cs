...
public static class HttpResponseHeaderEnumerableExtensions {
    ...
    public static string[] GetHeaderValues(this IEnumerable<HttpResponseHeader> headers, string name) => headers.GetHeaders(name).Select(h => h.Value).ToArray();

    public static string? GetHeaderValue(this IEnumerable<HttpResponseHeader> headers, string name) => headers.FirstOrDefault(h => h.Name.Equals(name, StringComparison.OrdinalIgnoreCase))?.Value;
}
